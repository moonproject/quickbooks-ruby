  ...
  module Service
    module ServiceCrud
      ...
      def all(object_query = nil, options = {})
        collection = []
        query_in_batches(object_query, options) do |batch|
      ...
      def find_by(field, selector, options = {})
        field = field.to_s.camelcase if field.class == Symbol
        q = format("select * from %s where %s = '%s'", model.resource_for_singular, field, selector)
        query(q, options)
      end
      ...
      def delete_by_query_string(entity, options = {})
        ...
        separator = resource_url.include?('?') ? '&' : '?'
