module Quickbooks
  module Model
    class Preferences < BaseModel
      XML_COLLECTION_NODE = 'Preferences'.freeze
      XML_NODE            = 'Preferences'.freeze
      REST_RESOURCE       = 'preferences'.freeze
      MINORVERSION = 32
      ...
      def self.create_preference_class(*attrs, &block)
        ::Class.new(BaseModel) do
          attrs.each do |a|
            xml_reader(a.underscore, from: a.delete('?'))
          end
      ...
      xml_reader :sales_forms, from: 'SalesFormsPrefs', as: create_preference_class('AllowDeposit?', 'AllowDiscount?', 'AllowEstimates?', 'AllowServiceDate?', 'AllowShipping?', 'AutoApplyCredit?', 'CustomField?', 'CustomTxnNumbers?', 'DefaultCustomerMessage', 'DefaultDiscountAccount?', 'DefaultShippingAccount?', 'DefaultTerms', 'EmailCopyToCompany?', 'EstimateMessage', 'ETransactionAttachPDF?', 'ETransactionEnabledStatus', 'ETransactionPaymentEnabled?', 'IPNSupportEnabled?', 'SalesEmailBcc', 'SalesEmailCc', 'UsingPriceLevels?', 'UsingProgressInvoicing?') {
