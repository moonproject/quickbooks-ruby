module Quickbooks
  module Model
    class TimeActivity < BaseModel
      XML_COLLECTION_NODE = 'TimeActivity'.freeze
      XML_NODE = 'TimeActivity'.freeze
      REST_RESOURCE = 'timeactivity'.freeze

      NAMEOF_OPTIONS = %w(Employee Vendor).freeze
      ...
      validate :existence_of_employee_ref, if: proc { |ta| ta.name_of == 'Employee' }
      validate :existence_of_vendor_ref, if: proc { |ta| ta.name_of == 'Vendor' }
