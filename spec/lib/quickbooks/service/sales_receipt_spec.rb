  ...
  module Service
    describe SalesReceipt do
      ...
      it 'queries for sales receipts' do
        ...
        stub_http_request(:get, subject.url_for_query, %w[200 OK], xml)
      ...
      it 'creates a sales receipt' do
        ...
        model = Model::SalesReceipt
        stub_http_request(:post, subject.url_for_resource(model::REST_RESOURCE), %w[200 OK], xml)
      ...
      it 'fetches sales receipt by ID' do
        xml = fixture('fetch_sales_receipt_by_id.xml')
        model = Model::SalesReceipt
        stub_http_request(:get, "#{subject.url_for_resource(model::REST_RESOURCE)}/1", %w[200 OK], xml)
      ...
      it 'can void a Sales Receipt' do
        ...
        stub_http_request(:post, "#{subject.url_for_resource(model::REST_RESOURCE)}?include=void", %w[200 OK], xml)
      ...
      it 'can send a sales receipt using bill_email' do
        ...
        model = Quickbooks::Model::SalesReceipt
        stub_http_request(:post, "#{subject.url_for_resource(model::REST_RESOURCE)}/1/send", %w[200 OK], xml)
        ...
        expect(sent_sales_receipt.delivery_info.delivery_time).to eq(Time.new(2015, 2, 24, 18, 26, 0o3, '-08:00'))
      end

      it 'can send an sales receipt with new email_address' do
        xml = fixture('sales_receipt_send.xml')
        model = Quickbooks::Model::SalesReceipt
        stub_http_request(:post, "#{subject.url_for_resource(model::REST_RESOURCE)}/1/send?sendTo=test@intuit.com", %w[200 OK], xml)
