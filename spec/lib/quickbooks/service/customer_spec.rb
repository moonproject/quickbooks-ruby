describe 'Quickbooks::Service::Customer' do
  ...
  it 'can fetch a customer by ID' do
    ...
    model = Quickbooks::Model::Customer
    stub_http_request(:get, "#{@service.url_for_base}/customer/1?minorversion=#{Quickbooks::Model::Customer::MINORVERSION}", %w[200 OK], xml)
  ...
  it 'cannot create a customer with an invalid display_name' do
    ...
    expect(customer.errors.key?(:display_name)).to eq(true)
  end
  ...
  it 'cannot create a customer with an invalid email' do
    ...
    expect(customer.valid?).to eq(false)
    expect(customer.errors.key?(:primary_email_address)).to eq(true)
  end
  ...
  it 'can sparse update a customer' do
    ...
    xml = fixture('fetch_customer_by_id.xml')
    stub_http_request(:post, @service.url_for_resource(model::REST_RESOURCE), %w[200 OK], xml, {}, true)
