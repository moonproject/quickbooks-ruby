describe Quickbooks::Service::TaxService do
  ...
  let(:tax_rate_details) do
    [Quickbooks::Model::TaxRateDetailLine.new(
  ...
  it 'can create tax code' do
    stub_http_request(:post, url, %w[200 OK], params.to_json)
  ...
  it 'catch exception if create tax code failed' do
    stub_http_request(:post, url, %w[200 OK], json_fixture(:tax_service_error_dup))
    tax_service.tax_rate_details = tax_rate_details
    expect do
      subject.create(tax_service)
    end.to raise_error(Quickbooks::IntuitRequestException, /The name supplied already exists/)
  end
