describe 'Quickbooks::Service::Batch' do
  ...
  it 'make batch request with success' do
    ...
    stub_http_request(:post, @service.url_for_resource('batch'), %w[200 OK], xml)
  ...
  it 'make batch request with error' do
    ...
    end.to raise_error(Quickbooks::IntuitRequestException)
  end

  it 'allows user to specify a RequestId in a create call' do
    requestid = 'foobar123'
    xml = fixture('batch_response.xml')
    stub_http_request(:post, "#{@service.url_for_resource('batch')}?requestid=#{requestid}", %w[200 OK], xml)
